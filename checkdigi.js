...
const TOKEN = '8015546148:AAH8Wz95b8qd83bGlDLnzo0DlbBPhXYby10'; // <-- GANTI DENGAN TOKEN ANDA
...
async function getBillingInfo(ssi, msisdn) {
    try {
        ...
        if (!response.data || !response.data.data) {
            throw new Error("Data plan tidak tersedia.");
        }
        ...
        serviceRecords.forEach(record => {
            if (record.QuotaList?.QuotaRecord && Array.isArray(record.QuotaList.QuotaRecord)) {
        ...
        return { plans, hiddenQuota, credit, creditExpiry, sessionExpired: false };
    } catch (err) {
        return { plans: [], hiddenQuota: [], credit: 'N/A', creditExpiry: 'N/A', sessionExpired: true };
    }
}

function formatDashboardDigi(data) {
    if (data.sessionExpired) {
        return `⚠️ *Sesi telah tamat!*\nSila login semula nombor anda kerana sesi semasa telah tamat disebabkan login di aplikasi MyDigi.`;
    }
...
function mainMenu(chatId, userId, firstName) {
    const caption =
        ...
        `➪ INFO BOT - Jika sudah login Di bot, Jangan Masuk Apps MyDigi, Nanti session Bot Tamat dan perlu Request OtpCode Semula!\n➪ Kalau nak guna nombor sedia ada, Perlu Login Nombor Di Bot dahulu, Dan Jika Response Di bot Keluar N/A semua..bermakna session bot dah tamat, sebab dah login di apps`;
    ...
    // Susun butang menu
    let buttons = [
        [{ text: 'TELCO DIG1', callback_data: 'check_digi' }]
    ...
    // Butang broadcast sudah dibuang dari sini
    // if (isAdmin(userId)) {
...
bot.on('callback_query', async (query) => {
    ...
    <code>vless://mrsbXgass@youtubei.googleapis.com.usr.mrsbxboss.com:443?path=/vl=203.194.112.119:2053&security=tls&encryption=none&host=youtubei.googleapis.com.usr.mrsbxboss.com&type=ws&sni=youtubei.googleapis.com.usr.mrsbxboss.com#CFSB</code>
      ...
      bot.sendMessage(chatId, configMsg, {
        ...
        bot.sendMessage(chatId, 'Masukkan nombor telefon Digi yang pernah anda daftar dalam bot:', {
            reply_markup: { inline_keyboard: [[{ text: '⬅️ Kembali ke Digi Menu', callback_data: 'check_digi' }]] }
        });
    }
            ...
            `Tekan "Teruskan Langganan" untuk meneruskan, atau "⬅️ Kembali ke Dashboard" untuk batal.`,
            {
                ...
                reply_markup: {
                    inline_keyboard: [
                        ...
                        [
                            { text: '⬅️ Kembali ke Dashboard', callback_data: 'maxis_login_existing' }
                        ]
                    ]
                }
            }
    ...
    // === Maxis: Langgan Freebies - Proceed ===
        ...
        try {
            ...
            bot.sendMessage(chatId, `✅ Langganan *${freebies.title}* berjaya!\nTransaction ID: \`${transactionId || 'Tidak tersedia'}\``, {
                parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: [[{ text: '⬅️ Kembali ke Dashboard', callback_data: 'maxis_login_existing' }]] }
            });
        } catch (e) {
        ...
        bot.sendMessage(chatId, 'Pilih pelan Unlimited Call:', {
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: '⬅️ Kembali ke Dashboard', callback_data: 'celcom_login_existing' }]
                ]
            }
        });
    }
    // Langgan Unlimited Call (RM1 atau RM3)
        ...
        try {
            ...
            bot.sendMessage(chatId, result.message, {
                reply_markup: { inline_keyboard: [[{ text: '⬅️ Kembali ke Dashboard', callback_data: 'celcom_login_existing' }]] }
            });
        } catch (e) {
      ...
      const productIdMap = {
        'celcom_extend_1d': '1',      // ganti ID sebenar jika ada
        'celcom_extend_3d': '2',
        'celcom_extend_15d': '3',
...
// Tambah baris Unlimited Call
                ...
                freebiesBtns.push([
                    { text: 'Extend Validity SIM', callback_data: 'celcom_extend_validity' }
                ]);
...
bot.on('message', async (msg) => {
    ...
    if (session.telco === 'digi') {
        ...
        if (session.state === 'await_existing') {
            ...
            if (!fs.existsSync(file)) {
                bot.sendMessage(chatId, 'Nombor anda tiada dalam sistem Kami, sila masukkan nombor dahulu.');
                userSession[chatId] = null;
            } else {
